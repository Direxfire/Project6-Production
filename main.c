...
char UART_Message_Global_Recieved[max_UART_Length];
int UART_Position_In_Counter = 0;
...
char Rolling_Average_ASCII[4] = "0000";
...
char Current_Temperature_ASCII[3] = "000";
/*
...
int main(void)
{
    ...
    */
    ...
    while (1)
    {
        int i;
//        / \e[?25l
        snprintf(UART_Message_Global, 100, "\e[H\033[2JSystem Starting \e[?25l");
        ...
        snprintf(UART_Message_Global, 100, "Loaded.\n\r");
        Send_UART_Message(6);
        for (i = 0; i < 20000; i++)
          {
          }
        ...
        snprintf(UART_Message_Global, 100, "\e[H\033[2J");
        Send_UART_Message(7);
        for (i = 0; i < 2000; i++)
          {
          }
		...
		snprintf(I2C_Message_Global,100, "%c", 'D');
	    Send_I2C_Message(LED, I2C_Message_Global_ptr, 1);

        //Send UART message for the inputs
        snprintf(UART_Message_Global, 100, "\rPlease enter the amount of samples to collect:  ");
        Send_UART_Message(53);
        while(Sample_Size == 0){
            ...
            if(New_Input_UART == 1){
                New_Input_UART = 0;
               *Sample_Size_ASCII_ptr = UART_Message_Global_Recieved[0];
                sscanf(Sample_Size_ASCII_ptr, "%d", &Sample_Size);
                snprintf(UART_Message_Global, 100, "%d Samples chosen\r\n\n", Sample_Size);
                Send_UART_Message(19);
                for (i = 0; i < 1000; i++);
        ...
        while(1){
            if(New_Input_UART == 1){
                ...
                switch(UART_Message_Global_Recieved[0]){
                case 'A':
                    Mode_Select_Char = UART_Message_Global_Recieved[0];
                    break;
                case 'a':
                    Mode_Select_Char = UART_Message_Global_Recieved[0] + 30;
                    break;
                case 'B':
                    Mode_Select_Char = UART_Message_Global_Recieved[0];
                    break;
                ...
                Peltier_On = 0;
                strncpy(Peltier_On_Char, "000", 3);
                snprintf(I2C_Message_Global,100, "%c", Mode_Select_Char);
                Send_I2C_Message(LED, I2C_Message_Global_ptr, 1);

                if(UART_Message_Global_Recieved[0] > '0' && UART_Message_Global_Recieved[0] < ':'){
                    Sample_Size = UART_Message_Global_Recieved[0] - '0';
                    sscanf(Sample_Size_ASCII_ptr, "%d", Sample_Size);
                    snprintf(UART_Message_Global, 100, "\n\rSample size updated to: %d.\r\n\n", Sample_Size);
                    Send_UART_Message(31);
            ...
            else if (New_Input == 1)
            {
                ...
                {
                    Sample_Size = Mode_Select - '0';
                    sscanf(Sample_Size_ASCII_ptr, "%d", Sample_Size);
                    snprintf(UART_Message_Global, 100, "\n\rSample size updated to: %d.\r\n", Sample_Size);
                    Send_UART_Message(30);

                }
                else if (Mode_Select > '@' && Mode_Select < 'E')
                {
                ...
                snprintf(I2C_Message_Global,100, "%c00.0%c%s00.0",Sample_Size + '0', Mode_Select_Char, Peltier_On_Char);
                Send_I2C_Message(LCD, I2C_Message_Global_ptr, 13);
            }
			// Set the mode indicator if it's A through D

        switch(Mode_Select_Char){
            case 'A':
                    // Heat the device
            ...
            if (Get_Time == 1)
            {
                ...
                if(Mode_Select_Char == 'C'){
                    Peltier_PID(Current_Temperature, Temperature_Array[Sample_Number].Celsius_Float);
                }
                Get_Time = 0;
                Read_Time();
                bcd_decimal();
                ...
                {
                    memset(UART_Message_Global, 0, 128);
                    snprintf(UART_Message_Global, 100, "\033[1A\r[20%d-%d-%dT%d:%d:%d-06:00]", Year, Month, Date, Hours, Minutes, Seconds);
                    Send_UART_Message(37);
                    for (i = 0; i < 1000; i++)
                        ;
                    ...
                    snprintf(UART_Message_Global, 100, "Ambient Temperature: %c%c.%c%cC. ", Rolling_Average_ASCII[0], Rolling_Average_ASCII[1], Rolling_Average_ASCII[2], 176);
                    Send_UART_Message(29);
                    for (i = 0; i < 1000; i++)
                        ;
                    snprintf(UART_Message_Global, 100, "Peltier Temperature is: %c%c.%c%cC. ", Current_Temperature_ASCII[0], Current_Temperature_ASCII[1], Current_Temperature_ASCII[2], 176);
                    Send_UART_Message(33);
                    for (i = 0; i < 1000; i++)
                        ;
                    snprintf(UART_Message_Global, 100, "Mode: ");
                    Send_UART_Message(6);
                    for (i = 0; i < 1000; i++)
                        ;
                    memset(UART_Message_Global, 0, 128);
                    ...
                    switch(Mode_Select_Char){
                    case 'A':
                        snprintf(UART_Message_Global, 100, "Heating.         \r\n");
                        Send_UART_Message(20);
                        break;
                    case 'B':
                        snprintf(UART_Message_Global, 100, "Cooling.          \r\n");
                        Send_UART_Message(20);
                        break;
                    case 'C':
                        snprintf(UART_Message_Global, 100, "Temp Matching.   \r\n");
                        Send_UART_Message(20);
                        break;
                    case 'D':
                        snprintf(UART_Message_Global, 100, "Idle.            \r\n");
                        Send_UART_Message(20);
                        break;
                    default:
                        snprintf(UART_Message_Global, 100, "\r\n");
                ...
                {
                    snprintf(UART_Message_Global, 100, "\r");
                    Send_UART_Message(1);
                    memset(UART_Message_Global, 0, 128);
                    snprintf(UART_Message_Global, 100, "\033[1AThe current time is:[20%d-%d-%dT%d:%d:%d-06:00]\r\n", Year, Month, Date, Hours, Minutes, Seconds);
                    Send_UART_Message(83);
                    for (i = 0; i < 2000; i++)
                        ;
                }
            }
...
__interrupt void ISR_EUSCI_A1(void)
{
    ...
    case 8:
        ...
        break;
    case 2:
        UART_Message_Global_Recieved[UART_Position_In_Counter] = UCA1RXBUF;
        if(UART_Message_Global_Recieved[UART_Position_In_Counter] == 13)
        {
            UART_Position_In_Counter = 0;
        }
        else{
            UART_Position_In_Counter++;
        }
        New_Input_UART = 1;
        break;
